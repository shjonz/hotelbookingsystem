...
const Header = ({type}) => {
    ...
    const handleSearch = () => {
        dispatch({type: "NEW_SEARCH", payload: {destination, dates, options, lang, currency, partner_id}})
        navigate("/hotels", { state: { destination, dates, options, dest_id } });
    };
    ...
    return (
        ...
        { type !== "list" && 
            ...
            <div className="headerSearchItem2">
                <span onClick={ () => setOpenDates( !openDates ) } className="headerSearchText1">{`${format(dates[0].startDate, "yyyy-MM-dd")} to 
